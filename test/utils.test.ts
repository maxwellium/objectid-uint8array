...
describe('utils isValid', () => {
  ...
  it('should detect valid objectIds', () => {
    expect(
      isValid(FIXTURE_VALID_UINT8ARRAY)
    ).toEqual(true);

    expect(
      isValid(FIXTURE_VALID_BINARY_STRING)
    ).toEqual(true);

    expect(
      isValid(FIXTURE_VALID_HEX_STRING)
    ).toBe(true)
  });

  it('should detect invalid objectIds', () => {
    expect(
      isValid(FIXTURE_INVALID)
    ).toEqual(false);
  });
...
describe('utils equals', () => {
  ...
  it('should detect equality of objectIds', () => {
    ...
    expect(
      equals(FIXTURE_VALID_UINT8ARRAY, FIXTURE_VALID_BINARY_STRING)
    ).toEqual(true);

    expect(
      equals(FIXTURE_VALID_HEX_STRING, FIXTURE_VALID_UINT8ARRAY)
    ).toEqual(true);

    expect(
      equals(FIXTURE_VALID_BINARY_STRING, FIXTURE_VALID_HEX_STRING)
    ).toEqual(true);

    expect(
      equals(FIXTURE_VALID_UINT8ARRAY, FIXTURE_VALID_UINT8ARRAY)
    ).toEqual(true);

    expect(
      equals(FIXTURE_VALID_UINT8ARRAY, FIXTURE_VALID_HEX_STRING_MUTATION)
    ).toEqual(false);
  });
  ...
  it('should throw when one of the arguments is maltyped', () => {
    ...
    expect(
      () => equals(FIXTURE_VALID_UINT8ARRAY, 1 as any as string)
    ).toThrowError('illegal argument types');
